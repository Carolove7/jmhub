...
class Page(HTMLParser):
    def __init__(self): super().__init__(); self.section = None; self.values = {"china": [], "flow1": [], "flow2": []}
    def handle_starttag(self, tag, attrs):
        cls = dict(attrs).get("class", "")
        for key, name in (("china", "china"), ("flow1", "first_line"), ("flow2", "second_line")):
            if name in cls: self.section = key
    def handle_endtag(self, tag):
        if tag in ("div", "section"): self.section = None
    def handle_data(self, data):
        if self.section:
            for value in re.findall(r"https?://[^\s<>\"']+", data):
                value = value.rstrip(".,，。)")
